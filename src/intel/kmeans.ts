...
export function fitStandardScaler(X: readonly (readonly number[])[]): StandardScaler {
  const n = X.length;
...
export function transformStandard(
  ...
  return X.map((row) => row.map((v, d) => (v - scaler.mean[d]) / scaler.scale[d]));
}
 ...
 */
export function silhouetteScore(X: readonly (readonly number[])[], labels: readonly number[]): number {
  const n = X.length;
  ...
  if (clusters.size < 2 || clusters.size >= n) {
    throw new Error(`silhouetteScore: needs 2 <= k < n (k=${clusters.size}, n=${n})`);
  }
  let total = 0;
  for (let i = 0; i < n; i++) {
