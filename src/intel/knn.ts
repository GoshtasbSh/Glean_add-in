...
export function knnDecide(rows: readonly NeighborRow[], opts: KnnOpts = {}): KnnDecision | null {
  ...
  if (share >= o.minAgreement && best >= o.threshold && best - runnerUp >= o.margin) {
...
export function knnClassify(
  ...
  const o = { ...DEFAULTS, ...opts };
  const ranking = items
    .map((it) => ({ id: it.id, label: it.label, sim: cosine(queryVec, it.vec) }))
