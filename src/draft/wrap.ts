...
function recencyWeight(lastUsed: string, now: Date): number {
  const ageDays = Math.max(0, (now.getTime() - new Date(lastUsed).getTime()) / MS_PER_DAY);
...
export function coldStartSignoff(userSignoffs: readonly LexiconEntry[], userFullName: string): string {
  let best = "";
  let bestScore = -1;
 ...
 */
export function selectGreeting(card: RelationshipCard, position: ThreadPosition, now: Date): string {
  const habit = card.threadGreetingHabit ?? { start: "greet" as const, mid: "none" as const };
  if ((position === "start" ? habit.start : habit.mid) === "none") return "";
  return selectLexicon(card.greetings, now) || coldStartGreeting(card.displayName);
}
...
export function wrapDraft(body: string, greeting: string, signoff: string): string {
  return [greeting, body.trim(), signoff].filter((part) => part.length > 0).join("\n\n");
}
