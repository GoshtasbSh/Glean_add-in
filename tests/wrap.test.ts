...
describe("selectGreeting", () => {
	it('card with "Dear Dr Von Meding," -> that exact greeting, always (DoD case)', () => {
		expect(selectGreeting(vonMedingCard, "start", NOW)).toBe(
			"Dear Dr Von Meding,",
		);
	});
	...
	it("mid-thread with habit mid=greet keeps the greeting", () => {
		const card = {
			...vonMedingCard,
			threadGreetingHabit: { start: "greet", mid: "greet" } as const,
		};
...
describe("cold start (no card) — design doc §1-MICRO", () => {
	it("greeting: Dear <Title LastName as in From-header>,", () => {
		expect(coldStartGreeting("Dr. Sarah Mitchell")).toBe(
			"Dear Dr. Sarah Mitchell,",
		);
		expect(coldStartGreeting("Jason Von Meding")).toBe(
			"Dear Jason Von Meding,",
		);
	});
	...
	it("signoff: most formal learned sign-off, else full name", () => {
		...
		expect(coldStartSignoff([], "Goshtasb Shahriari")).toBe(
			"Best regards,\nGoshtasb Shahriari",
		);
	});
});
...
describe("wrapDraft", () => {
	it("assembles [greeting]\\n\\n[body]\\n\\n[sign-off]", () => {
		expect(
			wrapDraft("Body text.", "Dear Dr Von Meding,", "Best regards,\nGoshtasb"),
		).toBe("Dear Dr Von Meding,\n\nBody text.\n\nBest regards,\nGoshtasb");
	});
