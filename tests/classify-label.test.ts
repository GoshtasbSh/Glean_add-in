...
describe("matchLabel", () => {
	it("matches an exact short name (case-insensitive)", () => {
		expect(matchLabel("To respond")?.name).toBe("Glean/To respond");
		expect(matchLabel("fyi")?.name).toBe("Glean/FYI");
	});

	it("matches when the model wraps the name in extra text", () => {
		expect(matchLabel("Label: Waiting.")?.name).toBe("Glean/Waiting");
		expect(matchLabel('"Meetings"')?.name).toBe("Glean/Meetings");
	});
...
describe("classifyLabel", () => {
	it("sanitizes + wraps the body and returns the chosen label", async () => {
		...
		expect(label?.name).toBe("Glean/To respond");
