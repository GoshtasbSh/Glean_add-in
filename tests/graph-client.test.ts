...
describe("graph client", () => {
	it("adds the bearer token and parses JSON", async () => {
		const fetchFn = vi
			.fn()
			.mockResolvedValue(jsonResponse(200, { displayName: "Albert" }));
	...
	it("on 401 refreshes the token once and retries", async () => {
		...
		const fetchFn = vi
			...
			.mockResolvedValueOnce(
				jsonResponse(401, { error: { code: "InvalidAuthenticationToken" } }),
			)
			.mockResolvedValueOnce(jsonResponse(200, { ok: true }));
		const client = createGraphClient({
			getToken,
		...
		const fetchFn = vi.fn().mockResolvedValue(
			jsonResponse(403, {
				error: {
					...
					message: "FERPA-sensitive secret body content",
				},
			}),
		);
		const client = createGraphClient({
	...
	it("caps the GraphError code at 64 characters", async () => {
		const fetchFn = vi
			.fn()
			.mockResolvedValue(
				jsonResponse(400, { error: { code: "x".repeat(200) } }),
			);
		const client = createGraphClient({
			getToken: async () => "tok",
			fetchFn,
	...
	it("returns the raw Response when opts.raw is set", async () => {
		...
		const res = await client.graph<Response>("GET", "/me/photo/$value", {
			raw: true,
		});
