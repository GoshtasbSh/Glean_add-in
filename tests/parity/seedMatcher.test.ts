...
describe("seed matcher parity", () => {
	it.each(
		oracle.labels.map((l) => [l.name, l] as const),
	)("label %s: query text, sims (1e-9), tagged exact", (_name, label) => {
		...
		expect(
			buildSeedQueryText(
				seed.name,
				seed.description,
				oracle.params.embed_prefix,
			),
		).toBe(label.query_text);
		...
		for (let i = 0; i < result.sims.length; i++) {
			expect(
				Math.abs(result.sims[i].sim - label.sims[i].sim),
	...
	it("DEFAULT_LABELS ports the legacy taxonomy (12 labels, 4 non-exclusive)", () => {
		...
		expect(
			DEFAULT_LABELS.filter((l) => !l.exclusive)
				.map((l) => l.name)
				.sort(),
		).toEqual(["Personal", "Research", "Service", "Teaching"]);
		expect(
			DEFAULT_LABELS.find((l) => l.name === "To Respond")?.needsDraft,
		).toBe(true);
	});
});
