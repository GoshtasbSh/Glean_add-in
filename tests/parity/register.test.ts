...
import { formalityScore } from "../../src/intel/features";
...
const corpus = (corpusJson as { emails: { id: string; to: string; text: string }[] }).emails;
...
describe("register parity", () => {
  ...
  it.each(oracle.edge_cases.map((c, i) => [i, c] as const))(
    "edge case %d (fail-formal + band boundaries)",
    (_i, c) => {
