...
const corpus = (
	corpusJson as { emails: { id: string; register: string; text: string }[] }
).emails;
...
describe("knn parity", () => {
	...
	it.each(
		oracle.queries.map((q) => [q.query_id, q] as const),
	)("query %s: ranking order, sims, decision all match", (_id, q) => {
		...
		for (let i = 0; i < q.top_k.length; i++) {
			...
			expect(Math.abs(result.topK[i].sim - q.top_k[i].sim)).toBeLessThanOrEqual(
				1e-9,
			);
		}
		if (q.decision === null) {
			...
			expect(
				Math.abs((result.decision?.confidence ?? 0) - q.decision.confidence),
...
describe("knnDecide gates (knn.py:77-97 semantics)", () => {
	...
	it("1-1 split vote fails the agreement gate (share 0.5 < 0.66)", () => {
		...
		expect(
			knnDecide(rows, { ...opts, minNeighbors: 2, margin: 0.06 }),
		).toBeNull();
	});
