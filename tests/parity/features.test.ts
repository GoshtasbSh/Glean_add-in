...
describe("features parity", () => {
	...
	it.each(
		oracle.emails.map((e) => [e.id, e] as const),
	)("styleFeatureVector(%s) matches oracle to 1e-9 per dimension", (_id, email) => {
		const vec = styleFeatureVector(email.text);
		expect(vec).toHaveLength(email.vector.length);
		for (let d = 0; d < vec.length; d++) {
			expect(
				Math.abs(vec[d] - email.vector[d]),
				`dim ${d} (${oracle.feature_names[d]})`,
	...
	it.each(
		oracle.emails.map((e) => [e.id, e] as const),
	)("formalityScore(%s) matches oracle exactly", (_id, email) => {
