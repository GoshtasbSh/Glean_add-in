...
import { fitStandardScaler, kmeansFit, kmeansPredict, silhouetteScore, transformStandard } from "../../src/intel/kmeans";
...
describe("standard scaler parity", () => {
  it("mean and scale match the oracle to 1e-9", () => {
    ...
    for (let d = 0; d < scaler.mean.length; d++) {
      expect(Math.abs(scaler.mean[d] - oracle.scaler_mean[d]), `mean dim ${d}`).toBeLessThanOrEqual(1e-9);
      expect(Math.abs(scaler.scale[d] - oracle.scaler_scale[d]), `scale dim ${d}`).toBeLessThanOrEqual(1e-9);
    }
  });
});
...
describe("fresh fit (quality band, documented exception)", () => {
  ...
  it("selectK degrades to 1 on sparse data (n < 2*minClusterSize)", () => {
    const X = transformStandard(raw.slice(0, 20), fitStandardScaler(raw.slice(0, 20)));
  ...
  it("centroids in original space match oracle when grouping by oracle assignments", () => {
    for (let c = 0; c < oracle.chosen_k; c++) {
      ...
      for (let d = 0; d < dim; d++) {
        ...
        expect(Math.abs(mean - oracle.centroids_original[c][d]), `cluster ${c} dim ${d}`).toBeLessThanOrEqual(1e-9);
      }
    }
  });
});
