...
import { T4_NEUTRAL_FORMAL, selectExemplars, type ExemplarPools } from "../src/intel/ladder";
...
describe("selectExemplars", () => {
  ...
  it("partial T1 fills from T2 then T3", () => {
    const pools: ExemplarPools = { t1: [ex("a")], t2: [ex("x"), ex("y")], t3: [ex("z")] };
    const out = selectExemplars(pools);
  ...
  it("dedupes identical bodies across tiers", () => {
    const out = selectExemplars({ t1: [ex("same")], t2: [ex("same"), ex("x")], t3: [ex("y")] });
    expect(out.map((e) => [e.tier, e.body])).toEqual([
      ["T1", "same"],
      ["T2", "x"],
      ["T3", "y"],
