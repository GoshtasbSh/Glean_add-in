...
import { createStore, ConflictError } from "../src/store/onedrive";
...
describe("schemas", () => {
  ...
  it("ProfileV1 rejects a wrong version", () => {
    expect(() => ProfileV1.parse({ ...PROFILE_SKELETON, version: 2 })).toThrow(ZodError);
  });
  ...
  it("SettingsV1 accepts its skeleton", () => {
    const s = { version: 1 as const, category_map: { "Glean/To respond": "preset0" }, project_rules: [] };
...
describe("onedrive store", () => {
  it("read returns parsed data and etag", async () => {
    const graphFn = vi.fn(async (method: string, path: string, opts?: { raw?: boolean }) => {
      if (path.endsWith(":/content") && opts?.raw) {
        return new Response(JSON.stringify(PROFILE_SKELETON), { status: 200 });
      }
      return { eTag: '"etag-1"' };
  ...
  it("read rejects corrupt JSON via schema validation", async () => {
    const graphFn = vi.fn(async (_m: string, path: string, opts?: { raw?: boolean }) => {
      if (path.endsWith(":/content") && opts?.raw) {
    ...
    const store = createStore(graphFn);
    await expect(store.read("profile.json", ProfileV1)).rejects.toThrow(ZodError);
  });

  it("write PUTs validated JSON and returns the new etag", async () => {
    const graphFn = vi.fn(async () => ({ eTag: '"etag-2"' })) as unknown as GraphFn;
    const store = createStore(graphFn);
    const { etag } = await store.write("profile.json", ProfileV1, PROFILE_SKELETON);
  ...
  it("write passes If-Match when an etag is given", async () => {
    const graphFn = vi.fn(async () => ({ eTag: '"etag-3"' })) as unknown as GraphFn;
    const store = createStore(graphFn);
