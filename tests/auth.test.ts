...
describe("createAuth", () => {
  it("configures MSAL with in-memory cache (custody rule)", async () => {
    ...
    expect((h.lastConfig as { cache: { cacheLocation: string } }).cache.cacheLocation).toBe(
      "memoryStorage"
    );
  });
  ...
  it("builds authority from the tenant env var", async () => {
    ...
    mockLoginPopup.mockResolvedValue({ account: FAKE_ACCOUNT });
    await auth.signIn();
    expect((h.lastConfig as { auth: { authority: string } }).auth.authority).toBe(
  ...
  it("getToken returns the silent token when silent succeeds", async () => {
    ...
    mockAcquireTokenSilent.mockResolvedValue({ accessToken: "silent-token-abc" });
    await auth.signIn();
  ...
  it("getToken falls back to popup on InteractionRequiredAuthError", async () => {
    ...
    mockLoginPopup.mockResolvedValue({ account: FAKE_ACCOUNT });
    mockGetActiveAccount.mockReturnValue(FAKE_ACCOUNT);
    mockAcquireTokenSilent.mockRejectedValue(new FakeInteractionRequiredAuthError("ui needed"));
