...
import { chat, chatStream, embed, NeedsKeyError } from "../src/llm/navigator";
...
describe("chatStream", () => {
	it("assembles deltas across split SSE frames and stops at [DONE]", async () => {
		...
		const full =
			chunk("Hel") + chunk("lo ") + chunk("world") + "data: [DONE]\n\n";
		const a = full.slice(0, 25);
		const b = full.slice(25);
		vi.stubGlobal(
			"fetch",
			vi.fn(async () => sseResponse([a, b])),
		);
		const out: string[] = [];
...
describe("chat", () => {
	it("returns the full completion content", async () => {
		...
		expect(
			await chat({ model: "m", system: "s", user: "u", retryBaseMs: 1 }),
		).toBe("full reply");
	});
});
